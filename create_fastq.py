...
def main(args):
    ...
    # read the gene distribution file / must be in the gene_locs
    genes, weights = read_genes_count_dist(args.gene_count_dist, gene_locs)
...
def read_genes_count_dist(gene_count_dist, gene_locs):
    genes =[]
    weights =[]
    with open(gene_count_dist, 'r') as fin:
       for x in fin.readlines():
         x = x.split('\t')
         if not x[0] in gene_locs:
            continue
         genes.append(x[0])
